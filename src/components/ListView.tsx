...
export default function ListView({ funds, handleSortClick }: ListViewProps) {
  return (
    <section className="overflow-x-auto" data-testid="list-view">
      <div className="grid grid-cols-9 text-left font-semibold pb-4 border-b border-b-light-gray italic">
        ...
        <div className=" text-center col-span-2">Launch date</div>
      </div>
      {funds.map((fund) => {
        const {
          id,
        ...
        return (
          <div key={id} className="pb-2 pt-4" data-testid="fund">
            <div
              className={`grid grid-cols-9 items-center bg-white shadow-md px-4 hover:shadow-xl border-l-4 ${regionColour}`}
            >
